...
def main(args):
    ...
    while episode < args.max_episodes:
        ...
        tmp_flag = True
        for w in env.env_core.map['objects']:
            if w.color == "grey" and tmp_flag:
                ckpt_reward.append(w.init_pos[-1])
                tmp_flag = False
            elif tmp_flag is not True:
                tmp_flag = True
        ...
        while True:
            ...
            # position reward
            agent_pos = env.env_core.agent_pos[ctrl_agent_index]
            for ck in ckpt_reward:
                if (agent_pos[0] - ck[0]) ** 2 + (agent_pos[1] - ck[1]) ** 2 < 400:
                    position_reward = -5 if ck in ckpt_reward_inverse else 5. if ck in ckpt_reward_tmp else 50.
                    if ck in ckpt_reward_tmp:
                        break
                    else:
                        ckpt_reward_tmp.append(ck)
            for ck in ckpt_reward_tmp:
                if (agent_pos[0] - ck[0]) ** 2 + (agent_pos[1] - ck[1]) ** 2 > 900:
                    ckpt_reward_inverse.append(ck)
            ...
            if not args.load_model:
                trans = Transition(obs_ctrl_agent, action_ctrl_raw, action_prob, post_reward[ctrl_agent_index]+position_reward,
